...
OIL_ORDER = ["Saudi Arabia", "United Arab Emirates", "Iraq"]
...
STYLE = (
    ...
    ".pct{font-size:12px;font-weight:760;fill:#ffffff}"
)
...
def build_rows() -> list[dict[str, str | float]]:
    ...
    for supplier in OIL_ORDER:
        baseline = oil[supplier]
        estimate = ESTIMATES[("oil", supplier)]
...
def write_svg(rows: list[dict[str, str | float]]) -> None:
    ...
    parts = [
        ...
        "</defs>",
        f"<style>{STYLE}</style>",
        '<text x="72" y="70" class="title">How much Hormuz-routed oil can still reach the world market?</text>',
    ...
    for row in rows:
        ...
        bar_y = y + 6
        supplier = "UAE" if row["supplier"] == "United Arab Emirates" else str(row["supplier"])
        parts.append(f'<text x="{label_x}" y="{y + 22}" class="label">{escape(supplier)}</text>')
        parts.append(f'<rect x="{bar_x}" y="{bar_y}" width="{bar_w}" height="{bar_h}" rx="3" fill="{COLORS["oil_soft"]}"/>')
        parts.append(f'<rect x="{bar_x}" y="{bar_y}" width="{bar_w}" height="{bar_h}" rx="3" fill="url(#oilHatch)"/>')
        parts.append(f'<rect x="{bar_x}" y="{bar_y}" width="{recoverable_w:.1f}" height="{bar_h}" rx="3" fill="{COLORS["oil"]}"/>')
        parts.append(f'<text x="{bar_x + recoverable_w - 8:.1f}" y="{y + 21}" text-anchor="end" class="pct">{share:.0%}</text>')
    ...
    parts.append(f'<rect x="72" y="428" width="1296" height="1" fill="{COLORS["line"]}"/>')
    parts.append('<text x="72" y="461" class="note">Sources: EIA/Vortexa, IEA, Aramco and ADNOC</text>')
    parts.append(
